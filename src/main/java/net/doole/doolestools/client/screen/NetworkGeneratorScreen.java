...
public class NetworkGeneratorScreen extends AbstractContainerScreen<NetworkGeneratorMenu> {
    ...
    public void extractBackground(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        g.text(font, "NETWORK GENERATOR", leftPos + 14, topPos + 14, DUTheme.TEXT_GREEN, false);
        ...
        g.text(font, "FUEL", leftPos + 16, topPos + 54, DUTheme.TEXT_DIM, false);

        // Status read-outs on the right, clear of the slot.
        g.text(font, menu.fePerTick() + " FE/t from fuel", leftPos + 48, topPos + 28, DUTheme.TEXT_DIM, false);

        int energyPct = menu.energyPercent();
        int burnPct = menu.burnPercent();
        g.text(font, "ENERGY", leftPos + 48, topPos + 40, DUTheme.TEXT_DIM, false);
        DUTheme.progress(g, leftPos + 88, topPos + 41, 52, 8, energyPct / 100f, DUTheme.PROGRESS_BLUE);
        g.text(font, energyPct + "%", leftPos + 144, topPos + 40, DUTheme.TEXT_DIM, false);

        g.text(font, "BURN", leftPos + 48, topPos + 52, DUTheme.TEXT_DIM, false);
        DUTheme.progress(g, leftPos + 88, topPos + 53, 52, 8, burnPct / 100f, burnPct > 0 ? DUTheme.PROGRESS_ORANGE : DUTheme.PANEL_ALT);
        g.text(font, burnPct > 0 ? burnPct + "%" : "IDLE", leftPos + 144, topPos + 52, burnPct > 0 ? DUTheme.WARN : DUTheme.TEXT_DIM, false);

        g.text(font, "Inventory", leftPos + 8, topPos + 72, DUTheme.TEXT_DIM, false);
